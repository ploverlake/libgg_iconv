...
gg_iconv_t gg_iconv_open(const char *to, const char *from)
{
	size_t f, t;
...
int gg_iconv_close(gg_iconv_t cd)
{
	return 0;
}

size_t gg_iconv(gg_iconv_t cd0, char **restrict in, size_t *restrict inb, char **restrict out, size_t *restrict outb)
{
	size_t x=0;
	...
	for (; *inb; *in+=l, *inb-=l) {
		...
		case UTF_8:
			l = gg_mbrtowc_utf8(&wc, *in, *inb, &st);
			if (!l) l++;
		...
		case BIG5:
			...
			if (c-0xa1>=0xfa-0xa1) {
				...
				if (c/256 == 0xdc) {
					...
					x += gg_iconv((gg_iconv_t)(uintptr_t)to,
		...
		case UTF_8:
			if (*outb < 4) {
				char tmp[4];
				k = gg_wctomb_utf8(tmp, c);
				if (*outb < k) goto toobig;
				memcpy(*out, tmp, k);
			} else k = gg_wctomb_utf8(*out, c);
			*out += k;
			*outb -= k;
			break;
		case US_ASCII:
